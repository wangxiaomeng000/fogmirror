...
async function analyzeImageWithGPT4o(imageBuffer) {
  try {
    ...
    console.log('使用GPT-4o-mini模型分析图片...');
...
请只描述你看到的，不要推测或想象。`
    ...
    console.log('GPT-4o分析成功，响应长度:', content.length);
    
    // 解析响应为结构化数据
    const analysis = parseAnalysis(content);
    analysis.分析状态 = 'gpt4o-success';
    analysis.原始描述 = content;
    
    return analysis;
  } catch (error) {
    console.error('GPT-4o分析失败:', error.response?.data || error.message);
...
app.post('/api/vision/analyze', upload.single('image'), async (req, res) => {
  try {
    ...
    const analysis = await analyzeImageWithGPT4o(req.file.buffer);
